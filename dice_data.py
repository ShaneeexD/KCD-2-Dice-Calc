"""
Kingdom Come: Deliverance 2 Dice Data Module
This module contains the dice data and classes to represent dice properties.
"""

class Die:
    """
    Class representing a die from Kingdom Come: Deliverance 2.
    """
    def __init__(self, name, file_name, item_id, description, weights, probabilities):
        """
        Initialize a die with its properties.
        
        Args:
            name (str): The name of the die as it appears in game
            file_name (str): Internal file name of the die
            item_id (str): UUID for the die in game
            description (str): Description of the die
            weights (list): List of weights for sides 1-6
            probabilities (list): List of probabilities (percentages) for sides 1-6
        """
        self.name = name
    ...
    def probability_of(self, number):
        """
        Get the probability of rolling a specific number.
        
        Args:
            number (int): The number on the die (1-6)
            
        Returns:
            float: Probability of rolling the number as a percentage
        """
        if 1 <= number <= 6:
...
# Dictionary to store all dice
ALL_DICE = {}

def load_dice_data():
    """
    Load all dice data from the predefined information.
    This function creates Die objects for each die in the game.
    
    Returns:
        dict: Dictionary of dice with names as keys and Die objects as values
    """
    # Dice percentage data (probability table)
    dice_probabilities = {
    ...
    # Dice weight data
    dice_weights = {
        "Aranka's die": [6, 1, 6, 1, 6, 1],
        "Cautious cheater's die": [5, 3, 2, 3, 5, 3],
        "Ci die": [3, 3, 3, 3, 3, 6],
        "Devil's head die": [1, 1, 1, 1, 1, 1],
        "Die of misfortune": [1, 5, 5, 5, 5, 1],
        "Even die": [2, 8, 2, 8, 2, 8],
        "Favourable die": [6, 0, 1, 1, 6, 4],
        "Fer die": [3, 3, 3, 3, 3, 5],
        "Greasy die": [3, 2, 3, 2, 3, 4],
        "Grimy die": [1, 5, 1, 1, 7, 1],
        "Grozav's lucky die": [1, 10, 1, 1, 1, 1],
        "Heavenly Kingdom die": [7, 2, 2, 2, 2, 4],
        "Holy Trinity die": [4, 5, 7, 1, 1, 1],
        "Hugo's Die": [1, 1, 1, 1, 1, 1],
        "King's die": [4, 6, 7, 8, 4, 3],
        "Lousy gambler's die": [2, 3, 2, 3, 7, 3],
        "Lu die": [3, 3, 3, 3, 3, 6],
        "Lucky Die": [6, 1, 2, 3, 4, 6],
        "Mathematician's Die": [4, 5, 6, 7, 1, 1],
        "Molar die": [1, 1, 1, 1, 1, 1],
        "Odd die": [8, 2, 8, 2, 8, 2],
        "Ordinary die": [1, 1, 1, 1, 1, 1],
        "Painted die": [3, 1, 1, 1, 6, 3],
        "Painter's die B": [1, 3, 2, 2, 2, 1],
        "Painter's die G": [1, 3, 2, 2, 2, 1],
        "Painter's die R": [1, 3, 2, 2, 2, 1],
        "Pie die": [6, 1, 3, 3, 0, 0],
        "Premolar die": [1, 1, 1, 1, 1, 1],
        "Sad Greaser's Die": [6, 6, 1, 1, 6, 3],
        "Saint Antiochus' die": [3, 1, 5, 1, 2, 3],
        "Shrinking die": [2, 1, 1, 1, 1, 3],
        "St. Stephen's die": [1, 1, 1, 1, 1, 1],
        "Strip die": [4, 2, 2, 2, 3, 3],
        "Three die": [2, 1, 4, 1, 2, 1],
        "Unbalanced Die": [3, 4, 1, 1, 2, 1],
        "Unlucky die": [1, 3, 2, 2, 2, 1],
        "Wagoner's Die": [1, 5, 6, 2, 2, 2],
        "Weighted die": [10, 1, 1, 1, 1, 1],
        "Wisdom tooth die": [1, 1, 1, 1, 1, 1]
    }
    ...
    # Create Die objects for each die
    dice_objects = {}
    for die_name in dice_probabilities:
        file_name, item_id, description = dice_info[die_name]
        weights = dice_weights[die_name]
        probabilities = dice_probabilities[die_name]
        dice_objects[die_name] = Die(
            die_name,
            file_name,
            item_id,
            description,
    ...
    global ALL_DICE
    ALL_DICE = dice_objects
    return dice_objects


def get_die_by_name(name):
    """
    Get a die by its name.
    
    Args:
        name (str): Name of the die
        
    Returns:
        Die: Die object if found, None otherwise
    """
    if not ALL_DICE:
        load_dice_data()
    return ALL_DICE.get(name)


def get_all_dice_names():
    """
    Get a list of all dice names.
    
    Returns:
        list: List of all dice names
    """
    if not ALL_DICE:
        load_dice_data()
    return list(ALL_DICE.keys())
